...
void WalkSupportStateDetector::loop() {
  config_ = param_listener_.get_params();
  // we only want to look at a window of the last n values which contains one global minimum
  ...
  std::vector<std::pair<float_t, rclcpp::Time>> pressure_times_left = pressure_left_values_stamped_;
  std::vector<std::pair<float_t, rclcpp::Time>> pressure_times_right = pressure_right_values_stamped_;
  ...
  // the minimum is where the foot switch happens
