...
def launch_setup(context, *args, **kwargs):
    ...
    robot_description_semantic_config = load_file(
        f"{robot_type.perform(context)}_moveit_config", f"config/{robot_type.perform(context)}.srdf"
    )
    ...
    ompl_planning_yaml = load_yaml(f"{robot_type.perform(context)}_moveit_config", "config/ompl_planning.yaml")
    ...
    rsp_node = Node(
        ...
        respawn=True,
        output="screen",
        parameters=[{"robot_description": robot_description, "publish_frequency": 100.0, "use_sim_time": sim}],
