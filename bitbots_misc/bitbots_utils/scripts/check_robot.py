...
from bitbots_msgs.msg import FootPressure, KickAction, KickGoal
...
if __name__ == "__main__":
    ...
    print_info(
        "First the motion software will be started. Please hold the robot, turn on servo power and press enter.\n"
    )
    ...
    launch = roslaunch.parent.ROSLaunchParent(
        uuid, [rospack.get_path("bitbots_bringup") + "/launch/motion_standalone.launch"]
    )
    launch.start()
    rclpy.sleep(5)
    while True:
    ...
    diag_sub = rclpy.Subscriber("diagnostics_toplevel_state", DiagnosticStatus, diagnostic_cb, tcp_nodelay=True)
    rclpy.sleep(3)
    ...
    topics_to_check = {
        ...
        "joint_states": 900,
        "robot_state": 900,
        "foot_pressure_left/raw": 900,
        "foot_pressure_left/filtered": 900,
        "foot_pressure_right/raw": 900,
        "foot_pressure_right/filtered": 900,
        "core/vdxl": 9,
        "diagnostics_toplevel_state": 9,
    }
    rts = []
    ...
    left_pressure_sub = rclpy.Subscriber(
        "foot_pressure_left/filtered", FootPressure, pressure_cb, callback_args=True, tcp_nodelay=True
    )
    right_pressure_sub = rclpy.Subscriber(
        "foot_pressure_right/filtered", FootPressure, pressure_cb, callback_args=False, tcp_nodelay=True
    )
    rclpy.sleep(0.5)
    ...
    if not both_okay:
        ...
        # call zero service
        zero_l = rclpy.ServiceProxy("foot_pressure_left/set_foot_zero", Empty)
        zero_r = rclpy.ServiceProxy("foot_pressure_right/set_foot_zero", Empty)
        zero_l()
        zero_r()
    ...
    print_info(
        "We will check the servo PWM status now. This gives information if any servo is going on maximum torque.\n"
    )
    ...
    # check walk motion
    walk_pub = rclpy.create_publisher(Twist, "/cmd_vel", 1)
    text = input_info(
    ...
    if text == "y":
        ...
        goal = KickGoal()
        goal.header.stamp = rclpy.get_clock().now()
