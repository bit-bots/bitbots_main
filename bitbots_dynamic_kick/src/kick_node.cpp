...
KickNode::KickNode(const std::string &ns) :
    ...
    if (!node_->get_parameter("robot_description_kinematics.LeftLeg.kinematics_solver", check_kinematic_parameters)) {
        ...
        while (!parameters_client->wait_for_service(1s)) {
            ...
            RCLCPP_INFO_THROTTLE(node_->get_logger(), *node_->get_clock(), 10*1e9, "Can't copy parameters from move_group node. Service not available, waiting again...");
        }
...
node_->get_parameter("unstable.stabilizing_point_y",  unstable_config_.stabilizing_point_y);
  ...
  cop_l_subscriber_ = node_->create_subscription<geometry_msgs::msg::PointStamped>("cop_l", 1, std::bind(&KickNode::copLCallback, this, _1));
  cop_r_subscriber_ = node_->create_subscription<geometry_msgs::msg::PointStamped>("cop_r", 1, std::bind(&KickNode::copRCallback, this, _1));
  joint_state_subscriber_ = node_->create_subscription<sensor_msgs::msg::JointState>("joint_states", 1, std::bind(&KickNode::jointStateCallback, this, _1));
  ...
  callback_handle_ = node_->add_on_set_parameters_callback(std::bind(&KickNode::onSetParameters, this, _1));
    ...
    server_ = rclcpp_action::create_server<bitbots_msgs::action::Kick>(node_,
                                                                       "dynamic_kick",
                                                                       std::bind(&KickNode::goalCb, this, std::placeholders::_1, std::placeholders::_2),
...
void KickNode::loopEngine(rclcpp::Rate loop_rate) {
  ...
  while (server_.isActive() && !server_.isPreemptRequested()) {
    ...
    if (loop_rate.sleep()) {
      ...
      bitbots_msgs::action::Kick::Feedback feedback;
      feedback.percent_done = engine_.getPercentDone();
      feedback.chosen_foot = engine_.isLeftKick() ?
                             bitbots_msgs::action::Kick::Feedback::FOOT_LEFT : bitbots_msgs::action::Kick::Feedback::FOOT_RIGHT;
