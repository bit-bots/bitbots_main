...
def generate_map_image(parameters):
    ...
    if target in [MapTypes.CROSSES, MapTypes.FIELD_FEATURES] and field_feature_style == FieldFeatureStyles.EXACT:
        # penalty marks
        if penalty_mark:
            drawCross(img, penalty_mark_left, color, stroke_width, field_feature_size // 2)
            drawCross(img, penalty_mark_right, color, stroke_width, field_feature_size // 2)
        ...
        drawCross(img, (middle_point[0], middle_point[1] - center_circle_diameter), color, stroke_width, field_feature_size // 2)
        drawCross(img, (middle_point[0], middle_point[1] + center_circle_diameter), color, stroke_width, field_feature_size // 2)
