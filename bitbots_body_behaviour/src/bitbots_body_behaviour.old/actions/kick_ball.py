...
class KickBall(AbstractActionElement):
    ...
    def __init__(self, blackboard, dsd, parameters=None):
        super(KickBall, self).__init__(blackboard, dsd)
        self.side = parameters
        self.begin = rospy.get_time()
        self.right_kick = blackboard.animation.config["Kicks"]["rightKick"]
        self.left_kick = blackboard.animation.config["Kicks"]["leftKick"]
        self.right_kick_strong = blackboard.animation.config["Kicks"]["rightKickStrong"]
        self.left_kick_strong = blackboard.animation.config["Kicks"]["leftKickStrong"]
        self.right_side_kick = blackboard.animation.config["Kicks"]["rightKickSide"]
        self.left_side_kick = blackboard.animation.config["Kicks"]["leftKickSide"]
    ...
    def perform(self, reevaluate=False):
        ...
        self.blackboard.blackboard.set_one_time_kicked(True)
        # todo make a better behaviour that looks if the ball realy moved

        if not self.blackboard.animation.is_animation_busy() and self.anim_begin:
        ...
        if self.side == "RIGHT_KICK":
            ...
            self.blackboard.animation.play_animation(self.left_kick)
        elif self.side == "RIGHT_KICK_STRONG":
            self.blackboard.animation.play_animation(self.right_kick_strong)
        elif self.side == "LEFT_KICK_STRONG":
            self.blackboard.animation.play_animation(self.left_kick_strong)
        elif self.side == "LEFT_SIDE_KICK":
            self.blackboard.animation.play_animation(self.left_side_kick)
        elif self.side == "RIGHT_SIDE_KICK":
            self.blackboard.animation.play_animation(self.right_side_kick)
        else:
