...
class AnimationCapsule(AbstractBlackboardCapsule):
    """Decides and communicates the optial positions, based on the game situation."""
    def __init__(self, node, blackboard):
        ...
        @dataclass
        class Field:
            length: float = parameters["field.size.x"]           # x extent (own goal at -length/2, opp goal at +length/2)
            width: float = parameters["field.size.y"]            # y extent
            goal_width: float = parameters["field.goal.width"]   # goal mouth
