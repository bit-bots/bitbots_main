...
class GoToDefensePosition(AbstractActionElement):
    ...
    def perform(self, reevaluate=False):
        ...
        #TODO index der Strategie
        self.strategie_index = 1
        ...
        #Strategies
        def position_freekick_first(index):
            if index == 1:
                ...
                #default positioning equal to index one
                vector_ball_to_goal = np.array(goal_position) - np.array(ball_position)
                # pos between ball and goal but 1.5m away from ball
                defense_pos = vector_ball_to_goal / np.linalg.norm(vector_ball_to_goal) * 1.5 + np.array(ball_position)
                pose_msg.pose.position.x = defense_pos[0]
        ...
        def position_freekick_second(index):
            if index == 1:
                vector_ball_to_goal = np.array(goal_position) - np.array(ball_position)
        ...
        def position_freekick_third(index):
            if index == 1:
        ...
        if self.mode == "freekick_first":
            if self.strategie_index == 1:
                position_freekick_first(1)
            else:
                position_freekick_first(1)
        elif self.mode == "freekick_second":
            if self.strategie_index == 1:
                position_freekick_second(1)
            else:
                position_freekick_second(1)
        elif self.mode == "freekick_third":
            if self.strategie_index == 1:
                position_freekick_third(1)
            else:
                position_freekick_third(1)
        else:
            position_freekick_third(1)
