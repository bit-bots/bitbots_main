...
class GoToFormationPosition(AbstractActionElement):
    ...
    def perform(self, reevaluate=False):
        ...
        assert not self.stand or self.on_position_threshold < self.off_position_threshold, (
            "on_position_threshold must be smaller than off_position_threshold"
        )
        assert not self.stand or self.on_orientation_threshold < self.off_orientation_threshold, (
            "on_orientation_threshold must be smaller than off_orientation_threshold"
        )

        if (
            self.stand
            and distance < self.on_position_threshold
            and angle_to_goal_orientation < self.on_orientation_threshold
        ):
            self.standing_on_target = True

        if self.standing_on_target and (
            distance < self.off_position_threshold and angle_to_goal_orientation < self.off_orientation_threshold
        ):
