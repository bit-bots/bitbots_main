...
void LocalObstacleMovementModel::drift(PositionStateW& state, geometry_msgs::Vector3 linear, geometry_msgs::Vector3 angular) const {
    ...
    double r = std::sqrt(std::pow(state.getXPos(), 2) + std::pow(state.getYPos(), 2)); // as this is done after the application of linear movement, the "robot" first walks linearly and turns then in this modelation.
    double d = std::atan(state.getXPos() / state.getYPos()); // make sure, you did not mix up x and y!!! The result is a radian
