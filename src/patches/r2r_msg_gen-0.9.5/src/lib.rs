...
#[repr(C)]
struct RosidlMessageTypeSupportPrefix {
    _typesupport_identifier: *const c_char,
    data: *const c_void,
}

unsafe fn type_support_data(ptr: *const rosidl_message_type_support_t) -> *const c_void {
    (*(ptr as *const RosidlMessageTypeSupportPrefix)).data
}
...
unsafe fn introspection<'a>(
    ...
    let members =
        type_support_data(ptr) as *const rosidl_typesupport_introspection_c__MessageMembers;
