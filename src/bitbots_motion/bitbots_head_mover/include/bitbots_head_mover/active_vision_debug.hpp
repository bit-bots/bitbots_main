...
///
/// Each candidate is drawn as the point on the ground its optical axis would aim
/// at, coloured by its score, which makes it visible at a glance whether the
/// planner is considering the right part of the field. The selected candidate is
/// drawn on top as a larger, distinct marker.
...
/// Plot the sampled candidate targets in the yaw and pitch joint space.
///
/// The horizontal axis is yaw and the vertical axis is pitch, each spanning the
/// configured joint limits. Every candidate is drawn as a dot coloured by its
/// score, with the selected one highlighted, so the shape of the search and the
/// score landscape can be judged directly rather than inferred from numbers.
