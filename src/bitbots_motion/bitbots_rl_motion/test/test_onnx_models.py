...
def test_walk_model_loadable():
    model_path = MODELS_DIR / "wolfgang_final_walking_ppo.onnx"
...
def test_walk_model_input_output_shape():
    ...
    assert inputs[0].type.tensor_type.shape.dim[1].dim_value == 49
    assert outputs[0].type.tensor_type.shape.dim[1].dim_value == 12
...
def test_kick_model_input_output_shape():
    ...
    assert inputs[0].type.tensor_type.shape.dim[1].dim_value == 48
    assert outputs[0].type.tensor_type.shape.dim[1].dim_value == 12
...
def test_kick_model_runnable():
    ...
    dummy_input = np.zeros((1, 48), dtype=np.float32)
    result = session.run(None, {"in_0": dummy_input})
