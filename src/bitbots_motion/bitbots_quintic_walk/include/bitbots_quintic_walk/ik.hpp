 ...
 */
static Mat3 euler2mat_sxyz(double a, double b, double c) { return Rz(c) * Ry(b) * Rx(a); }
 ...
 */
static Mat3 euler2mat_rzxy(double a, double b, double c) { return Rz(a) * Rx(b) * Ry(c); }
 ...
 */
static Mat3 euler2mat_rxyz(double a, double b, double c) { return Rx(a) * Ry(b) * Rz(c); }
...
static Mat4 foot_to_leg_from_vec_subproblem(const Vec3& leg_vector_in) {
  ...
  if (std::abs(v(0)) > 0.99 || std::abs(v(1)) > 0.99) {
    throw SolverError("Leg vector is too close to the x or y axis, cannot determine roll and pitch.");
  }
...
static JointAngles calculate_ik(const Mat4& target_transform, bool left) {
  JointAngles joint_angles = {{"HipYaw", 0.0}, {"HipRoll", 0.0},    {"HipPitch", 0.0},
                              {"Knee", 0.0},   {"AnklePitch", 0.0}, {"AnkleRoll", 0.0}};
  ...
  auto target_transform_leg_only =
      (T_base_link_to_hip_intersection.inverse() * target_transform) * T_ankle_to_sole.inverse();
  ...
  T_hip_without_pitch.block<3, 3>(0, 0) = euler2mat_rzxy(joint_angles["HipYaw"], joint_angles["HipRoll"], 0.0);
  ...
  Mat4 T_real_leg = T_ankle_axis_intersection.inverse() * T_hip_axis_intersection * T_hip_pitch_origin;
  ...
  double knee_arg =
      (real_leg_length * real_leg_length - UPPER_LEG_LENGTH * UPPER_LEG_LENGTH - LOWER_LEG_LENGTH * LOWER_LEG_LENGTH) /
  ...
  double hip_pitch_offset_arg =
      (UPPER_LEG_LENGTH * UPPER_LEG_LENGTH + real_leg_length * real_leg_length - LOWER_LEG_LENGTH * LOWER_LEG_LENGTH) /
  ...
  double ankle_pitch_offset_arg =
      (LOWER_LEG_LENGTH * LOWER_LEG_LENGTH + real_leg_length * real_leg_length - UPPER_LEG_LENGTH * UPPER_LEG_LENGTH) /
...
static JointAngles calculate_ik_right_leg(const Mat4& target_transform) {
  auto ik_solution = calculate_ik(target_transform, false);
...
static JointAngles calculate_ik_left_leg(const Mat4& target_transform) {
  auto ik_solution = calculate_ik(target_transform, true);
