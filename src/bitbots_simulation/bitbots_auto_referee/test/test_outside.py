...
def test_goal_height_sides_and_own_goal():
    ...
    for last_touch in (1, 2, None):
        ...
        assert (decision.kind, decision.team_id) == ("GOAL", 1)
    decision = field.classify((4.0, 0.0, 2.0), (5.0, 0.0, 2.0), match, 1, 0)
    ...
    assert decision.team_id == 2


def test_restart_awards_and_first_crossing():
    field = FieldGeometry()
    match = state()
    corner = field.classify((4.0, 2.0, 0.1), (5.0, 2.0, 0.1), match, 2, 0)
    assert (corner.kind, corner.team_id, corner.position) == ("SET_PLAY_CORNER_KICK", 1, (4.5, 3.0))
    kick = field.classify((4.0, -2.0, 0.1), (5.0, -2.0, 0.1), match, 1, 0)
    assert (kick.kind, kick.team_id, kick.position) == ("SET_PLAY_GOAL_KICK", 2, (3.5, -1.5))
...
def test_goal_delay_acknowledgement_and_ready_set_playing():
    ...
    assert match.teams[0].score == 0
    commands.teleport_ball.assert_not_called()
    match = checker.check_rules(match, sample(3, (5.0, 0.0, 0.1)))
    assert match.teams[0].score == 1 and match.stopped and match.kicking_team == 2
...
def test_failed_placement_does_not_repeat_score_or_command():
    ...
    assert match.stopped and match.teams[0].score == 1
