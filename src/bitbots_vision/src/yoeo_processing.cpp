...
std::vector<float> preprocess_image(const cv::Mat& bgr, int net_h, int net_w, PreprocessInfo& info) {
  ...
  info.pad_bottom = (info.orig_w > info.orig_h) ? (info.orig_w - info.orig_h) - info.pad_top : 0;
  info.pad_left = (info.orig_h > info.orig_w) ? (info.orig_h - info.orig_w) / 2 : 0;
  info.pad_right = (info.orig_h > info.orig_w) ? (info.orig_h - info.orig_w) - info.pad_left : 0;
  ...
  cv::Mat padded;
  cv::copyMakeBorder(rgb_float, padded, info.pad_top, info.pad_bottom, info.pad_left, info.pad_right,
...
std::vector<int> nms_boxes(const std::vector<cv::Rect2d>& boxes, const std::vector<float>& scores,
                           ...
                           float nms_threshold, int max_detections) {
  ...
  for (size_t i = 0; i < boxes.size(); ++i) {
    ...
    if (!robot_class_ids.empty()) {
      bool is_robot = std::find(robot_class_ids.begin(), robot_class_ids.end(), eff) != robot_class_ids.end();
      if (is_robot) {
...
std::unordered_map<std::string, std::vector<Candidate>> postprocess_detections(
  ...
  const float scale = static_cast<float>(info.max_dim) / static_cast<float>(info.net_h);
  ...
  for (int idx : keep) {
    ...
    result[class_names[class_ids[idx]]].push_back(Candidate::from_x1y1x2y2(
        static_cast<int>(x1), static_cast<int>(y1), static_cast<int>(x2), static_cast<int>(y2), scores[idx]));
  }
  return result;
}
...
std::unordered_map<std::string, cv::Mat> postprocess_segmentation(const uint8_t* data, int seg_h, int seg_w,
  ...
  cv::Mat seg_float(seg_h, seg_w, CV_8UC1, const_cast<uint8_t*>(data));

  cv::Mat seg_padded;
  cv::resize(seg_float, seg_padded, cv::Size(info.max_dim, info.max_dim), 0, 0, cv::INTER_NEAREST);

  cv::Mat seg_cropped = seg_padded(cv::Range(info.pad_top, info.max_dim - info.pad_bottom),
  ...
  for (int i = 0; i < static_cast<int>(class_names.size()); ++i) {
    result[class_names[i]] = (seg_cropped == i);
  }
  return result;
}
