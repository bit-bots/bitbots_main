...
{
  const auto providers = Ort::GetAvailableProviders();
  bool has_cpu = std::find(providers.begin(), providers.end(), "CPUExecutionProvider") !=
                 providers.end();
...
TEST(GetAvailableProviders, KnownProviderNames)
{
  ...
  bool has_cuda = std::find(providers.begin(), providers.end(), "CUDAExecutionProvider") !=
                  providers.end();
...
static std::vector<std::string> select_providers(
