...
static std::vector<float> make_det_row(float xc, float yc, float w, float h, float obj_conf,
...
TEST(PostprocessDetections, BelowObjConfThreshold_Filtered) {
  ...
  auto result =
      postprocess_detections(row.data(), 1, static_cast<int64_t>(row.size()), {"ball", "robot"}, {1}, 0.5f, 0.4f, info);
  EXPECT_TRUE(result.empty());
}
...
TEST(PostprocessDetections, NMS_SuppressesOverlappingBoxes) {
  ...
  PreprocessInfo info{416, 416, 416, 0, 0, 0, 0, 416, 416};
  auto result = postprocess_detections(data.data(), 2, static_cast<int64_t>(row1.size()), {"ball", "robot"}, {1}, 0.5f,
                                       0.4f, info);
...
TEST(PostprocessSegmentation, MaskCountMatchesClassCount) {
  std::vector<uint8_t> data(16 * 16, 0.f);
  PreprocessInfo info{16, 16, 16, 0, 0, 0, 0, 16, 16};

  auto result = postprocess_segmentation(data.data(), 16, 16, {"background", "field", "lines"}, info);
...
TEST(PostprocessSegmentation, MaskSizeMatchesOriginal) {
  ...
  std::vector<uint8_t> data(8 * 8, 0.f);
  PreprocessInfo info{150, 100, 150, 0, 0, 25, 25, 8, 8};

  auto result = postprocess_segmentation(data.data(), 8, 8, {"background", "field"}, info);
...
TEST(PostprocessSegmentation, AllClass0_Class0MaskAllActive) {
  ...
  const int H = 8, W = 8;
  std::vector<uint8_t> data(H * W, 0.f);
  PreprocessInfo info{H, W, W, 0, 0, 0, 0, H, W};

  auto result = postprocess_segmentation(data.data(), H, W, {"background", "field"}, info);
...
TEST(PostprocessSegmentation, AllClass1_Class1MaskAllActive) {
  const int H = 8, W = 8;
  std::vector<uint8_t> data(H * W, 1.f);  // all pixels → class 1
  PreprocessInfo info{H, W, W, 0, 0, 0, 0, H, W};
...
TEST(PostprocessSegmentation, MixedClasses_MutuallyExclusiveMasks) {
  ...
  for (int r = 0; r < H; ++r) {
    for (int c = 0; c < W; ++c) {
      if (c < W / 2) {
        EXPECT_GT(result.at("bg").at<uint8_t>(r, c), 0u) << "bg should be active at (" << r << "," << c << ")";
        EXPECT_EQ(result.at("field").at<uint8_t>(r, c), 0u) << "field should be inactive at (" << r << "," << c << ")";
      } else {
        EXPECT_EQ(result.at("bg").at<uint8_t>(r, c), 0u) << "bg should be inactive at (" << r << "," << c << ")";
        EXPECT_GT(result.at("field").at<uint8_t>(r, c), 0u) << "field should be active at (" << r << "," << c << ")";
      }
    }
  }
}
