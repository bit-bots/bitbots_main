...
bool LedsHardwareInterface::init() {
  if (start_number_ == 0) {
    leds_service_ = nh_->create_service<bitbots_msgs::srv::Leds>("/set_leds", &LedsHardwareInterface::setLeds);
  }
  sub0_ = nh_->create_subscription<std_msgs::msg::ColorRGBA>("/led" + std::to_string(start_number_),
                                                             1, std::bind(&LedsHardwareInterface::ledCb0, this, _1));
  sub1_ = nh_->create_subscription<std_msgs::msg::ColorRGBA>("/led" + std::to_string(start_number_ + 1),
                                                             1, std::bind(&LedsHardwareInterface::ledCb1, this, _1));
  sub2_ = nh_->create_subscription<std_msgs::msg::ColorRGBA>("/led" + std::to_string(start_number_ + 2),
                                                             1, std::bind(&LedsHardwareInterface::ledCb2, this, _1));
  return true;
}
...
bool LedsHardwareInterface::setLeds(bitbots_msgs::srv::Leds::Request &req, bitbots_msgs::srv::Leds::Response &resp) {
  if (req.leds.size() != leds_.size()) {
    RCLCPP_WARN_STREAM(nh_->get_logger(),
                       "You are trying to set " << req.leds.size() << " leds while the board has " << leds_.size()
                                                << " leds.");
    return false;
  }
