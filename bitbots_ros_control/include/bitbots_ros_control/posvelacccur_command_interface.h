...
class PosVelAccCurJointHandle : public PosVelAccJointHandle {
 public:
   ...
   */
  PosVelAccCurJointHandle(const JointStateHandle &js,
                          double *cmd_pos,
                          double *cmd_vel,
                          double *cmd_acc,
                          double *cmd_cur)
    ...
    if (!cmd_cur) {
      throw HardwareInterfaceException(
          "Cannot create handle '" + js.getName() + "'. Command current data pointer is null.");
    }
  }
  ...
  void setCommandCurrent(double cmd_cur) {
    assert(cmd_cur_);
    *cmd_cur_ = cmd_cur;
  }
  double getCommandCurrent() const {
    assert(cmd_cur_);
    return *cmd_cur_;
  }
