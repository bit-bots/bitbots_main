...
class RecordUI(Plugin):
    ...
    def state_update(self, joint_states):
        ...
        for i in range(0, len(joint_states.name)):
            #if (not self._motorSwitched[joint_states.name[i]]):     This causes the program to crash when using the checkboxes.
    ...
    def box_ticked(self):
        ...
        for k, v in self._treeItems.items():
            if self._motorSwitched[k] != (v.checkState(0) == 2): #if motor stiffness should change
                msg.joint_names.append(k)
        ...
        if len(msg.joint_names) > 0:
            #print(len(msg.joint_names))
            #print(msg)
            self._joint_pub.publish(msg)

        print(len(self._motorSwitched.items()))
