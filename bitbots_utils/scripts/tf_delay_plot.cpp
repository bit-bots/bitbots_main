    ...
    public:
        TfDelayPlot() : Node("tf_delay_plot", rclcpp::NodeOptions()) {
            ...
            tf_sub_ = this->create_subscription<tf2_msgs::msg::TFMessage>(
                "/tf",
                5,
                std::bind(&TfDelayPlot::tf_callback, this, _1));
        }
        ...
        void tf_callback(const tf2_msgs::msg::TFMessage::SharedPtr msg) {
            ...
            for (auto &transform: msg->transforms) {
                // Calculate delay
                double delay = ((transform.header.stamp.sec * 1.0e9 + transform.header.stamp.nanosec) - this->now().nanoseconds()) / 1.0e9;
                ...
                if (std::abs(delay) > 0.0001) {
                    ...
                    // Print stamp
                    RCLCPP_INFO(this->get_logger(), "Stamp: %f", transform.header.stamp.sec + transform.header.stamp.nanosec / 1.0e9);
                }
            }
        }
