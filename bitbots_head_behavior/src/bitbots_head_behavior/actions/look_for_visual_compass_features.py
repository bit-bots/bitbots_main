...
class LookForVisualCompassFeatures(AbstractActionElement):
    """
    Looks for features for the visual compass above the field boundary
    """

    def __init__(self, blackboard, dsd, parameters=None):
        super(LookForVisualCompassFeatures, self).__init__(blackboard, dsd, parameters)
        
        self.index = 0

        head_mode = self.blackboard.head_capsule.head_mode

        pattern_config = self.blackboard.config['visual_compass_features_pattern']
