...
class AbstractLookAt(AbstractActionElement):
    ...
    def _look_at(self, point):
        ...
        try:
            ...
            rospy.logerr(f'Could not find transform {self.head_tf_frame}. Either it does not exist or '
                         f'transform is not yet online.\n{e}')
            return
        except tf2.ConnectivityException as e:
            rospy.logerr(f'No connection to transform\n{e}')
            return
...
class LookDirection(AbstractLookAt):
    ...
    def __init__(self, blackboard, dsd, parameters=None):
        ...
        assert parameters['direction'] in dir(self.Directions), f'Direction {parameters["direction"]} not found'
