...
show_help() {
    ...
    echo "  run-project [ip]         Run project container (uses port $SSH_PORT_PROJECT or specified IP)"
    echo "  run-target [ip]          Run target container (uses port $SSH_PORT_TARGET or specified IP)"
    echo "  run-robot <id>           Run target container with IP from known_targets.yaml (id: hostname/robot_name/IP)"
...
run_robot() {
    ...
    if [ -z "$identifier" ]; then
        echo "Usage: $0 run-robot <hostname|robot_name|IP>"
        exit 1
    fi

    # Resolve IP from known_targets.yaml
    local ip=$(python3 -c "
import yaml
import sys
try:
    ...
    subnet_prefix = '${DEFAULT_SUBNET%.*}'
    
    matches = []
    for ip, info in data.items():
        if info.get('hostname', '').lower() == ident or info.get('robot_name', '').lower() == ident or ip == ident:
            matches.append(ip)
    
    # Prefer IP in our subnet
    found_ip = None
    for m in matches:
        if m.startswith(subnet_prefix):
            found_ip = m
            break
...
")
    ...
    if [ -z "$ip" ]; then
        # Check if identifier is already an IP
        if [[ $identifier =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
            ip=$identifier
        else
            echo "Could not find IP for identifier: $identifier"
            exit 1
        fi
    fi
