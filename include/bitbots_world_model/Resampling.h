...
template <class StateType>
ImportanceResamplingWE<StateType>::ImportanceResamplingWE(int explorer_count,
        std::shared_ptr<particle_filter::StateDistribution<StateType>>
                distribution) :
...
void ImportanceResamplingWE<StateType>::resample(const ParticleList& sourceList,
    ...
    for (unsigned int destIndex = resample_max;
            destIndex < destinationList.size(); destIndex++) {
