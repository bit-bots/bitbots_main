    ...
    for (size_t i=1; i<points_.size(); i++) {
        ...
        if (time > 0.00001) {
            Spline::splines_.push_back({
                                           polynomFit(time,
                                                      points_[i-1].position, points_[i-1].velocity, points_[i-1].acceleration,
                                 ...
                                 double pos_2, double vel_2, double acc_2) const
{
    ...
    p.getCoefs()[3] = -(-acc_2*t_2+3*acc_1*t_2+8*vel_2*t+12*vel_1*t-20*pos_2+20*pos_1)/(2*t_3);
    p.getCoefs()[4] = (-2*acc_2*t_2+3*acc_1*t_2+14*vel_2*t+16*vel_1*t-30*pos_2+30*pos_1)/(2*t_4);
