...
OdometryFuser::OdometryFuser() : Node("OdometryFuser"),
  ...
  while (!tf_buffer_->canTransform(l_sole_frame_, base_link_frame_, rclcpp::Time(0), rclcpp::Duration(1))
      && rclcpp::ok()) {
...
tf2::Transform OdometryFuser::getCurrentRotationPoint() {
  ...
  tf_buffer_->canTransform(r_sole_frame_, l_sole_frame_, fused_time_, rclcpp::Duration(0.1));
