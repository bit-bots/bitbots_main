...
class Vision:
    ...
    def _configure_vision(self, config, level):
        ...
        if ros_utils.config_param_change(self._config, config, 'vision_publish_field_mask_image'):
            ...
            if self._publish_field_mask_image:
                rospy.loginfo('(Dynamic color space-) Field mask image publishing is enabled', logger_name="dynamic_color_lookup_table")
            else:
                rospy.loginfo('(Dynamic color space-) Field mask image publishing is disabled', logger_name="dynamic_color_lookup_table")
    ...
    def _register_or_update_all_subscribers(self, config):
        ...
        if self._use_dynamic_color_space:
            self._sub_dynamic_color_space_msg_topic = ros_utils.create_or_update_subscriber(self._config, config, self._sub_dynamic_color_space_msg_topic, 'ROS_dynamic_color_space_msg_topic', ColorLookupTable, callback=self._field_color_detector.color_space_callback, queue_size=1, buff_size=2 ** 20)
