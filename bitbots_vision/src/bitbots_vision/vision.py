...
class Vision:
    def __init__(self):
        ...
        self.add_model_enums(VisionConfig)
        self.add_color_space_enums(VisionConfig)
    ...
    def add_model_enums(self, cfg_type):
        """
        ...
        models_directory = os.path.join(self.package_path, "models")
        ...
        Vision.change_enum_items(cfg_type, 'fcnn_model_path', fcnn_paths)
        Vision.change_enum_items(cfg_type, 'yolo_model_path', yolo_paths)

    def add_color_space_enums(self, cfg_type):
        """
        Add models to dynamic reconfigure enums.
        ...
        color_spaces_path = os.path.join(self.package_path, "config/color_spaces")
        ...
        # Add enums to configuration
        Vision.change_enum_items(cfg_type, 'field_color_detector_path', field_color_space_enum)
        Vision.change_enum_items(cfg_type, 'field_color_detector_path_sim', field_color_space_enum)

    @staticmethod
    def change_enum_items(cfg_type, parameter_name, new_items, default=None):
        """
        ...
        if not Vision.recursive_search(cfg_type, cfg_type.config_description['groups'], parameter_name, new_items, default):
            # Raise exception if param is not in config
            raise RuntimeError('Parameter "%s" not found.' % parameter_name)

    @staticmethod
    def recursive_search(cfg_type, groups, parameter_name, new_items, default):
        """
        ...
        for group in groups:
            ...
            if Vision.process_group(cfg_type, group, parameter_name, new_items, default):
                # Notify that item has been set
                return True
            # Search in the subgroups
            if Vision.recursive_search(cfg_type, group['groups'], parameter_name, new_items, default):
                # Notify that item has been set
                return True

    @staticmethod
    def process_group(cfg_type, group, parameter_name, new_items, default):
