...
try:
    ...
    logger.warning("Not able to run Darknet YOLO! Its only executable under python3 with yolo34py or yolo34py-gpu installed.")
try:
    from openvino.inference_engine import IENetwork, IECore
except ImportError:
    logger.warning("Not able to run YOLO on the Intel NCS2 TPU! The OpenVINO SDK should be installed if you intend to run YOLO on the TPU")
try:
    ie = IECore()
except NameError:
    logger.warning("Please install/source OpenVino environment to use the NCS2 YOLO Handler.")
try:
    from pytorchyolo import models as torch_models, detect as torch_detect
except ImportError:
    logger.warning("Not able to import pytorchyolo. This might be fine if you use another method.")
...
class YoloHandlerNCS2(YoloHandler):
    ...
    def __init__(self, config, model_path):
        ...
        logger.debug(f"Loading network files:\n\t{model_xml}\n\t{model_bin}")
